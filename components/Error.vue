<template>
	<div class="font-sans antialiased bg-white dark:bg-black text-black dark:text-white grid min-h-screen place-content-center overflow-hidden">
		<div class="fixed top-60 left-0 right-0 spotlight z-10"></div>
		<div class="max-w-520px text-center z-20">
			<h1 class="text-8xl sm:text-10xl font-medium mb-8" v-text="statusCode" />
			<p class="text-xl px-8 sm:px-0 sm:text-4xl font-light mb-16 leading-tight" v-text="description" />
...
useHead({
	title: `${props.appName} | ${props.statusCode} - ${props.statusMessage}`,
	script: [],
	style: [
		{
...
<style scoped>
.spotlight {
	background: linear-gradient(45deg, #f7bad8 0%, #f0c1f9 50%, #afc7fc 100%);
	filter: blur(20vh);
	height: 20vh;
	bottom: -30vh;
}
